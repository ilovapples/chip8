...
pub fn printHighlightLineError(writer: *std.Io.Writer, log_level: LogLevel, comptime fmt: []const u8, fmt_args: anytype, context: StringContextInfo) void {
    if (with_ansi_color) {
...
pub fn customDataPrintHighlightLineError(writer: *std.Io.Writer, data: *const [4]LogLevelCodes, log_level: LogLevel, comptime fmt: []const u8, fmt_args: anytype, context: StringContextInfo) void {
